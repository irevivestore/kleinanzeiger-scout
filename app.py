...
if seite == "🔍 Aktive Anzeigen":
    st.markdown("## 🔍 Aktive Kleinanzeigen")
    ...
    if submit:
        ...
        gespeicherte = sum(
            not is_advert_archived(anzeige["id"]) and not save_advert(anzeige) for anzeige in neue_anzeigen
        )
        if gespeicherte:
    ...
    for anzeige in alle_anzeigen:
        ...
        with st.container():
            ...
            with col2:
                ...
                with col_save:
                    with st.markdown('<div class="primary-button">', unsafe_allow_html=True):
                        if st.button("📂 Speichern", key=f"save_{anzeige['id']}"):
                            update_manual_defekt_keys(anzeige["id"], json.dumps(defekte_select))
                            st.rerun()
                with col_archive:
                    with st.markdown('<div class="archive-button">', unsafe_allow_html=True):
...
# Archivierte Anzeigen identisch (kannst du auf Wunsch ebenfalls in Cards umbauen, wenn du möchtest)
